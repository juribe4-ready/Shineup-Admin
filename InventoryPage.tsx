...
import {
  ...
  Clock, Filter, AlertTriangle, CheckCircle, Zap
} from 'lucide-react'
...
interface TimelineGroup {
  ...
  avgDurationMin: number | null; avgEstimatedMin: number | null
  onTimeRate: number | null; lateCount: number
}
...
// ─── Squad Metrics Panel ──────────────────────────────────────────────────────
function SquadMetrics({ timeline }: { timeline: TimelineGroup[] }) {
  // Calculate global metrics
  const totalCleanings = timeline.reduce((sum, g) => sum + g.total, 0)
  const totalDone = timeline.reduce((sum, g) => sum + g.done, 0)
  const globalPct = totalCleanings > 0 ? Math.round((totalDone / totalCleanings) * 100) : 0
  
  // Average rating across all squads with ratings
  const squadsWithRating = timeline.filter(g => g.avgRating !== null)
  const globalAvgRating = squadsWithRating.length > 0
    ? squadsWithRating.reduce((sum, g) => sum + (g.avgRating || 0), 0) / squadsWithRating.length
    : null

  // Global average duration
  const squadsWithDuration = timeline.filter(g => g.avgDurationMin !== null)
  const globalAvgDuration = squadsWithDuration.length > 0
    ? Math.round(squadsWithDuration.reduce((sum, g) => sum + (g.avgDurationMin || 0), 0) / squadsWithDuration.length)
    : null
  const globalAvgEstimated = squadsWithDuration.filter(g => g.avgEstimatedMin).length > 0
    ? Math.round(squadsWithDuration.filter(g => g.avgEstimatedMin).reduce((sum, g) => sum + (g.avgEstimatedMin || 0), 0) / squadsWithDuration.filter(g => g.avgEstimatedMin).length)
    : null

  // Global on-time rate
  const squadsWithOnTime = timeline.filter(g => g.onTimeRate !== null)
  const globalOnTimeRate = squadsWithOnTime.length > 0
    ? Math.round(squadsWithOnTime.reduce((sum, g) => sum + (g.onTimeRate || 0), 0) / squadsWithOnTime.length)
    : null
  const totalLate = timeline.reduce((sum, g) => sum + (g.lateCount || 0), 0)

  // Top/bottom performers by completion %
  const sortedByPct = [...timeline]
    .filter(g => g.total > 0)
    .sort((a, b) => (b.done / b.total) - (a.done / a.total))
  
  const topSquad = sortedByPct[0]
  const bottomSquad = sortedByPct.length > 1 ? sortedByPct[sortedByPct.length - 1] : null

  // Extract first cleaner name for display
  const getSquadName = (g: TimelineGroup) => {
    const cleaners = g.cleanings[0]?.staffList.filter(s => {
      const role = (s.role || '').toLowerCase()
      return role.includes('cleaner') || role === ''
    }) || []
    return cleaners.length > 0 ? cleaners[0].name.split(' ')[0] : g.staffListText.split(',')[0]?.trim() || 'Squad'
  }
  ...
  return (
    <div className="rounded-3xl overflow-hidden shadow-sm" style={{ background: C.white, border: `1px solid ${C.border}` }}>
      <div className="px-5 py-4" style={{ borderBottom: `1px solid ${C.border}` }}>
        <p className="font-black text-[14px]" style={{ color: C.ink }}>Métricas por Grupo</p>
        <p className="text-[12px] font-medium mt-0.5" style={{ color: C.muted }}>{timeline.length} grupos activos</p>
      </div>
      
      {/* Main metrics grid */}
      <div className="p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {/* Global completion */}
        <div className="p-3 rounded-2xl" style={{ background: globalPct === 100 ? '#DCFCE7' : C.bg }}>
          <p className="text-[9px] font-black uppercase tracking-widest mb-1" style={{ color: C.muted }}>Avance</p>
          <p className="font-black text-[26px] leading-none" style={{ color: globalPct === 100 ? C.green : C.primary }}>{globalPct}%</p>
          <p className="text-[10px] font-medium mt-1" style={{ color: C.slate }}>{totalDone}/{totalCleanings}</p>
        </div>

        {/* Global avg rating */}
        <div className="p-3 rounded-2xl" style={{ background: globalAvgRating && globalAvgRating >= 2.5 ? '#DCFCE7' : globalAvgRating ? '#FEF3C7' : C.bg }}>
          <p className="text-[9px] font-black uppercase tracking-widest mb-1" style={{ color: C.muted }}>Rating</p>
          {globalAvgRating !== null ? (
            <div className="flex items-center gap-1">
              <span className="font-black text-[26px] leading-none" style={{ color: globalAvgRating >= 2.5 ? C.green : globalAvgRating >= 1.5 ? C.amber : C.red }}>
                {globalAvgRating.toFixed(1)}
              </span>
              <span className="text-[16px]">⭐</span>
            </div>
          ) : (
            <p className="font-black text-[16px] leading-none" style={{ color: C.muted }}>--</p>
          )}
        </div>

        {/* Average duration vs estimated */}
        <div className="p-3 rounded-2xl" style={{ background: globalAvgDuration && globalAvgEstimated && globalAvgDuration <= globalAvgEstimated ? '#DCFCE7' : globalAvgDuration ? '#FEF3C7' : C.bg }}>
          <div className="flex items-center gap-1 mb-1">
            <Clock className="w-3 h-3" style={{ color: C.muted }} />
            <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: C.muted }}>Tiempo Real</p>
          </div>
          {globalAvgDuration !== null ? (
            <>
              <p className="font-black text-[20px] leading-none" style={{ color: globalAvgDuration <= (globalAvgEstimated || 999) ? C.green : C.amber }}>
                {formatDuration(globalAvgDuration)}
              </p>
              {globalAvgEstimated && (
                <p className="text-[10px] font-medium mt-1" style={{ color: C.slate }}>
                  Est: {formatDuration(globalAvgEstimated)}
                </p>
              )}
            </>
          ) : (
        ...
        {/* On-time rate */}
        <div className="p-3 rounded-2xl" style={{ background: globalOnTimeRate && globalOnTimeRate >= 80 ? '#DCFCE7' : globalOnTimeRate ? '#FEF3C7' : C.bg }}>
          <div className="flex items-center gap-1 mb-1">
          ...
          {globalOnTimeRate !== null ? (
            <>
              <p className="font-black text-[26px] leading-none" style={{ color: globalOnTimeRate >= 80 ? C.green : globalOnTimeRate >= 60 ? C.amber : C.red }}>
                {globalOnTimeRate}%
              </p>
              {totalLate > 0 && (
                <p className="text-[10px] font-medium mt-1" style={{ color: C.amber }}>{totalLate} tarde</p>
              )}
            </>
          ) : (
            <p className="font-black text-[16px] leading-none" style={{ color: C.muted }}>--</p>
        ...
        {topSquad && (
          <div className="p-3 rounded-2xl" style={{ background: '#EEF2FF' }}>
            <div className="flex items-center gap-1 mb-1">
              ...
              <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: C.muted }}>Líder</p>
            </div>
            <p className="font-black text-[16px] leading-none truncate" style={{ color: C.primary }}>{getSquadName(topSquad)}</p>
            <p className="text-[10px] font-medium mt-1" style={{ color: C.slate }}>
              {Math.round((topSquad.done / topSquad.total) * 100)}%
            </p>
          </div>
        )}

        {/* Bottom performer */}
        {bottomSquad && bottomSquad !== topSquad && (
          <div className="p-3 rounded-2xl" style={{ background: '#FEF3C7' }}>
            <div className="flex items-center gap-1 mb-1">
              <AlertTriangle className="w-3 h-3" style={{ color: C.amber }} />
              <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: C.muted }}>Atención</p>
            </div>
            <p className="font-black text-[16px] leading-none truncate" style={{ color: C.amber }}>{getSquadName(bottomSquad)}</p>
            <p className="text-[10px] font-medium mt-1" style={{ color: C.slate }}>
              {Math.round((bottomSquad.done / bottomSquad.total) * 100)}%
            </p>
          </div>
        )}
      </div>
      ...
      <div className="px-4 pb-4">
        <div className="p-3 rounded-2xl" style={{ background: C.bg }}>
          <p className="text-[9px] font-black uppercase tracking-widest mb-3" style={{ color: C.muted }}>Detalle por Grupo</p>
          <div className="space-y-2">
            {timeline.map(g => {
              const pct = g.total > 0 ? Math.round((g.done / g.total) * 100) : 0
              return (
                <div key={g.staffListText} className="flex items-center gap-3 p-2 rounded-xl" style={{ background: C.white }}>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold truncate" style={{ color: C.ink }}>{getSquadName(g)}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] font-semibold" style={{ color: pct === 100 ? C.green : C.slate }}>{pct}%</span>
                      {g.avgRating && (
                        <span className="text-[10px]" style={{ color: g.avgRating >= 2.5 ? C.green : C.amber }}>⭐{g.avgRating.toFixed(1)}</span>
                      )}
                      {g.avgDurationMin && (
                        <span className="text-[10px]" style={{ color: C.muted }}>⏱{formatDuration(g.avgDurationMin)}</span>
                      )}
                      {g.onTimeRate !== null && (
                        <span className="text-[10px]" style={{ color: g.onTimeRate >= 80 ? C.green : C.amber }}>⚡{g.onTimeRate}%</span>
                      )}
                    </div>
                  </div>
                  {/* Mini progress bar */}
                  <div className="w-16 h-1.5 rounded-full overflow-hidden" style={{ background: C.border }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? C.green : C.primary }} />
                  </div>
                </div>
              )
            })}
      ...
      <div className="overflow-x-auto" style={{ position: 'relative' }}>
        <div style={{ minWidth: '700px' }}>
          ...
          {timeline.map(group => {
            ...
            // Filter to only show cleaner roles in avatars
...
export default function DashboardPage({ profile: _profile }: Props) {
  ...
  const [ganttFilter, setGanttFilter] = useState<'all' | 'inProgress' | 'late' | 'done'>('all')
  ...
  // Filter timeline based on ganttFilter
  const filteredTimeline = data?.timeline.map(group => {
    if (ganttFilter === 'all') return group
    const filteredCleanings = group.cleanings.filter(c => {
      if (ganttFilter === 'inProgress') return c.status === 'In Progress' || c.status === 'Opened'
      if (ganttFilter === 'done') return c.status === 'Done'
      if (ganttFilter === 'late') {
        if (!c.scheduledTime || !c.startTime) return false
        const sched = new Date(c.scheduledTime).getTime()
        const start = new Date(c.startTime).getTime()
        return (start - sched) / 60000 > 15
      }
      return true
    })
    return { ...group, cleanings: filteredCleanings, total: filteredCleanings.length }
  }).filter(g => g.cleanings.length > 0) || []

  return (
    <div className="space-y-6">

      {/* Stats + Date selector + LIVE indicator */}
          ...
          <div className="flex items-center gap-3">
            ...
            {date === today() && (
              <div className="flex items-center gap-1.5 px-2 py-1 rounded-full" style={{ background: '#DCFCE7' }}>
                ...
                <span className="text-[10px] font-black uppercase tracking-wide" style={{ color: C.green }}>Live</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 mt-0.5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {[
              ...
              { key: 'inProgress', label: 'En Progreso', icon: '▶' },
              { key: 'late', label: 'Atrasados', icon: '⚠️' },
              { key: 'done', label: 'Terminados', icon: '✓' },
            ].map(f => (
              <button
                key={f.key}
                onClick={() => setGanttFilter(f.key as typeof ganttFilter)}
            ...
            {ganttFilter !== 'all' && (
              <span className="text-[11px] font-medium ml-2" style={{ color: C.muted }}>
                {filteredTimeline.reduce((sum, g) => sum + g.cleanings.length, 0)} resultados
              </span>
            )}
          </div>
      ...
      {/* Squad Metrics */}
      {data && data.timeline.length > 0 && <SquadMetrics timeline={data.timeline} />}
