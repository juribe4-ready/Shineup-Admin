...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Auto-fill staffing: the squad is now known for this cleaning on this date, so resolve
    // "who's actually on this squad today" (override-or-default) and stamp it onto the Cleaning.
    // Best-effort — a failure here shouldn't fail the block creation itself.
    if (cleaningId) {
      try {
      ...
      } catch (e) { console.error('[createSquadBlock] auto-staff fill error:', e.message) }
    }
