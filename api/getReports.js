...
import { computeJobDurationMinutes, resolveRating } from './_lib/duration.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // TARS config (Standards/TARS_CONFIG) — single source of truth for the duration formula's
    // tunables (round-to, minimum, rating adjustments). See api/_lib/duration.js.
    let tarsConfig = {}
    try {
      const cfgRes = await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE}/Standards?filterByFormula={Name}='TARS_CONFIG'&fields[]=Value`, { headers: { 'Authorization': `Bearer ${AIRTABLE_TOKEN}` } })
      if (cfgRes.ok) {
        const cfgData = await cfgRes.json()
        const raw = cfgData.records?.[0]?.fields?.Value
        if (raw) tarsConfig = JSON.parse(raw)
      }
    } catch (e) { console.error('[getDashboard] tarsConfig fetch error:', e.message) }
    ...
    const cleanings = filtered.map(record => {
      ...
      if (labor > 0 && f['Scheduled Time']) {
        ...
        const totalMinutes = computeJobDurationMinutes(labor, cleanerCount, ratingVal, tarsConfig)
